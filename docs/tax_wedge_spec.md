...
> adjudication); implementation queued.** Governs a ledger-computed diagnostic
> in the `account_size_floor` / `carry_flatten` pattern: one script, one
> tracked JSON, one doc section. Searches nothing, moves no ratified
> statistic, appends no trials-ledger row.
